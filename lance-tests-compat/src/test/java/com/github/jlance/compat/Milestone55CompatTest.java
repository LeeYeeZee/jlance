...
/**
 * Compatibility test for list&lt;struct&lt;nullable list&lt;int&gt;, nullable string&gt;&gt;.
 */
public class Milestone55CompatTest {

  private static final Path DATA_DIR = Paths.get("..", "compat_tests", "data", "milestone_55");

  @Test
  public void testListStructListNullable() throws Exception {
    Path file = DATA_DIR.resolve("test_list_struct_list_nullable.lance");
    try (BufferAllocator allocator = new RootAllocator();
      ...
      try (VectorSchemaRoot root = reader.readBatch(allocator)) {
        ...
        assertFalse(outerList.isNull(5));
        assertEquals(1, outerList.getElementEndIndex(5) - outerList.getElementStartIndex(5));
        ...
        // Struct 2 (row 1): {"items": null, "name": "null_items"}
        assertEquals(0, innerList.getElementEndIndex(2) - innerList.getElementStartIndex(2));
        assertTrue(innerList.isNull(2));
        assertEquals("null_items", nameVec.getObject(2).toString());

        // Struct 3 (row 4): {"items": [4, 5], "name": null}
        assertEquals(2, innerList.getElementEndIndex(3) - innerList.getElementStartIndex(3));
        assertEquals(4, innerData.get(3));
        assertEquals(5, innerData.get(4));
        assertTrue(nameVec.isNull(3));

        // Struct 4 (row 5): {"items": null, "name": null}
        assertEquals(0, innerList.getElementEndIndex(4) - innerList.getElementStartIndex(4));
        assertTrue(innerList.isNull(4));
        assertTrue(nameVec.isNull(4));
      }
    }
  }
}
